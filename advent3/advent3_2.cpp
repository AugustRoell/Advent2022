...
//item categories occuring at least once in each triplet of backpacks.
//Each such item is the badge that the triplet of elves should be 
//carrying.
...
int main() {
	...
	//The string objects that will contain the inventories of each
	//of the three elves in any group.
	...
	//unsigned integer objects containing the sum of all the 
	//priorities of the badges.
	...
	//
	//NOTE: it may provide a marginal speed-up to first find out which 
	//of the three elves has the smallest inventory and then loop over
	//that string, instead of looping over the first inventory 
	//regardless of size.
	for (int i = 0; i < 100; i++) {
		std::cin >> backpack_1;
		std::cin >> backpack_2;
		...
		for (char item : backpack_1) {
			if (backpack_2.find(item) != std::string::npos && 
					backpack_3.find(item) != std::string::npos) {
	...
	std::cout << "The sum of priorities of the bagdes is " << priorities_sum << '\n';
