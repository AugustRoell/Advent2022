...
//categories, mistakenly placed in both compartments of an elf's backpack.
...
int main() {
	...
	//The string objects that will contain a line from the input, the 
	//characters corresponding to the items in the first compartment of 
	//the backpack, and similarly those in the second compartment.
	...
	//of even length), after which we loop over the characters in the 
	//first half of the string, checking every time if this character also 
	//occurs in the second half of the string. If it does, we attribute a 
	//value, called the priority, to this character by finding its index 
	//in the alphabet list above and adding 1 (so that 'a' has value 1). 
	//After that we break the iteration over characters, to prevent double 
	//occurences.
	for (int i = 0; i < 300; i++) {
		std::cin >> line;
	...
	std::cout << "The sum of priorities of the displaced items is " << 
		priorities_sum << '\n';
